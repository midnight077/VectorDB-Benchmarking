...
_CLASS_MAP = {
    Metric.L2: _vdbhnsw.HnswFastL2,
    Metric.INNER_PRODUCT: _vdbhnsw.HnswFastIP,
    Metric.COSINE: _vdbhnsw.HnswFastIP,
}


class HnswFastAdapter(VectorDBAdapter):
    """Custom from-scratch HNSW: VCL SIMD distances, serial build (Phase B),
    single-thread search.

    See CUSTOM_HNSW_PLAN.md Phase B. Same graph algorithm and build/search
    params as hnsw_scalar (M, ef_construction, ef_search) -- only the
    distance kernel differs, so recall@k should match hnsw_scalar exactly at
    matched params while search latency improves.
    """
    ...
    def build_index(self, vectors: np.ndarray, ids: np.ndarray) -> BuildResult:
        ...
        # Phase B: build path is still serial (OpenMP lands in Phase C).
